...
class LightingConfig:
    ...
    RAY_STEP_SIZE = 8  # Distance between ray checks (smaller = more accurate but slower)
...
class FlashlightSystem:
    ...
    def create_light_polygon(self, player_screen_pos, player_world_pos, facing_direction):
        """
    ...
    def draw_player_glow(self, surface, player_screen_pos):
        ...
        for r in range(glow_radius, 0, -4):
            ...
            alpha = max(0, self.config.GLOW_BASE_ALPHA - (alpha_ratio * self.config.GLOW_BASE_ALPHA))
            pygame.draw.circle(
                glow_surface,
